...
/// The stochastic-rounding seed of one tree: training seed, boosting round
/// and output index mixed so every tree draws independent rounding noise.
pub(crate) fn rounding_seed(seed: u64, round: u64, output: u64) -> u64 {
    mix64(mix64(seed ^ 0x7175_616E_745F_6772) ^ round.wrapping_mul(GOLDEN) ^ mix64(output))
}
...
mod tests {
    ...
    #[test]
    fn rounding_seeds_differ_across_trees() {
        let a = rounding_seed(0, 0, 0);
        assert_ne!(a, rounding_seed(0, 1, 0));
        assert_ne!(a, rounding_seed(0, 0, 1));
        assert_ne!(a, rounding_seed(1, 0, 0));
        assert_eq!(a, rounding_seed(0, 0, 0));
    }
}
