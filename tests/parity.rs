...
use hessboost::prelude::{
    ...
    ProcessType, TrainingParams, TreeMethod, train, train_continue,
};
...
impl Case<'_> {
    ...
    /// Continuation fixtures train `first_rounds` and continue the model to
    /// `num_round`, as XGBoost's `xgb_model=` run did.
    fn train_and_compare(&mut self, dtest: &DMatrix) -> Result<(BoostedModel, Vec<f32>), String> {
        ...
        let model = match &fx.continuation {
            None => train(&params, &dtrain, fx.num_round).map_err(|e| format!("train: {e}"))?,
            Some(c) => {
                let first =
                    train(&params, &dtrain, c.first_rounds).map_err(|e| format!("train: {e}"))?;
                train_continue(&params, &dtrain, fx.num_round - c.first_rounds, &first)
...
fn xgboost_parity() {
    ...
    println!(
        "{:<30} {:<7} {:<22} {:<9} {:<9} {:<9} {:<5} {:<8} {:<11} base_score hessboost | xgboost",
        "case", "tier", "train", "import", "margin", "contribs", "ubj", "export", "extra"
    );
