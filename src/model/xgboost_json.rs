...
//! models — one per alpha for `reg:quantileerror` / `reg:expectileerror`,
//! whose [`BoostedModel::n_targets`] is the single label column — and
//! [`BoostedModel::n_targets`] (1) for multiclass. Import checks it against
//! the rebuilt objective's output count.
//!
//! ## UBJSON encoding
//!
...
fn objective_to_json(objective: &str, num_class: usize, params: &ObjectiveParams) -> Value {
    let alphas = match objective {
        // XGBoost's `MeanAbsoluteError::SaveConfig` writes the name only.
        "reg:absoluteerror" => return json!({ "name": objective }),
        "reg:quantileerror" => Some((QUANTILE_ALPHA, &params.quantile_alpha)),
        "reg:expectileerror" => Some((EXPECTILE_ALPHA, &params.expectile_alpha)),
        _ => None,
    };
    if let Some(((block, key), alpha)) = alphas {
        return json!({ "name": objective, block: { key: format_param_array(alpha) } });
    }
