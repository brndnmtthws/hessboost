...
//!   **export** the sequoia model to `fixtures/exports/` for
...
use sequoia_boost::data::HistCuts;
use sequoia_boost::{
    train, BoostedModel, BoosterKind, DMatrix, FeatureType, GrowPolicy, Monotone, SequoiaError,
...
/// A parameter sequoia implements only one way: the fixture must carry exactly
...
fn expect_fixed(key: &str, v: &Value, want: &Value) -> Result<(), String> {
    if v == want {
        ...
        Err(format!(
            "`{key}` must be {want} (sequoia's only implementation), got {v}"
        ))
    }
}
...
fn build_params(fx: &Fixture) -> Result<TrainingParams, String> {
    ...
    for (key, v) in &fx.params {
        ...
        b = match k {
            ...
            // gblinear: sequoia implements exactly cyclic coordinate descent.
            "updater" => {
            ...
            // rank:*: sequoia pairs every document with every other in the
...
fn max_abs_diff(what: &str, a: &[f32], b: &[f32]) -> Result<f64, String> {
    if a.len() != b.len() {
        return Err(format!(
            "{what}: length mismatch (sequoia {}, xgboost {})",
            a.len(),
            b.len()
        ));
    }
...
impl Case<'_> {
    ...
    fn train_and_compare(&mut self, dtest: &DMatrix) -> Result<(BoostedModel, Vec<f32>), String> {
        ...
        if preds.len() != fx.xgb_pred.len() {
            return Err(format!(
                "train predict: length mismatch (sequoia {}, xgboost {})",
                preds.len(),
                fx.xgb_pred.len()
            ));
        }
    ...
    fn quality_band(&mut self, preds: &[f32]) -> String {
        ...
        if !ok {
            self.fail(format!(
                "quality band: {metric} sequoia={seq:.5} xgboost={xgb:.5} (band {band})"
            ));
        }
    ...
    /// `(rows, num_class, n_cols + 1)` is identical to sequoia's
    ...
    fn import_and_compare(&mut self, dtest: &DMatrix, dcontrib: &DMatrix) -> [String; 3] {
        ...
        if booster_of(fx) == "gblinear" {
            return match imported {
                Err(SequoiaError::ModelFormat(_)) => std::array::from_fn(|_| "n/a".to_string()),
                Err(e) => {
    ...
    /// Assertion 3: export the trained model plus sequoia's predictions for
...
fn xgboost_parity() {
    ...
    println!(
        "{:<26} {:<7} {:<22} {:<9} {:<9} {:<9} {:<8} base_score sequoia | xgboost",
    ...
    for (_, fx) in &fixtures {
        ...
        for f in 0..fx.n_cols {
            ...
            let problem = match first_diff {
                Some(i) => Some(format!(
                    "feature {f} cut {i}: sequoia {:e} ({:#010x}) vs xgboost {:e} ({:#010x})",
                    seq[i],
                    seq[i].to_bits(),
                    xgb[i],
                    xgb[i].to_bits()
